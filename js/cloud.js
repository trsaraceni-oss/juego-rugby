    ...
    return {
      ...
      async signUpPassword(email, password, name) {
        ...
        if (!data || !data.access_token) {
          /* El servidor no dice «ese mail ya existe», para no contarle a nadie
             quién tiene cuenta: devuelve un alta vacía, sin identidades. Si no,
             mandábamos a revisar un mail que nunca se iba a mandar. */
          const yaEsta = data && data.user && Array.isArray(data.user.identities) &&
            data.user.identities.length === 0;
          if (yaEsta) {
            throw new Error('Ya hay una cuenta con ese mail. Entrá con tu contraseña, o pedí un ' +
              'link si no la tenés.');
          }
          /* el proyecto pide confirmar el mail: recién ahí hay que ir al correo */
          return { user: null, confirmar: true, email: clean };
        }
